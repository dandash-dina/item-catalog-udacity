...
def fbconnect():
    ...
    # Gets information  from facebook clients secrets
...
def gconnect():
    ...
    output += login_session['picture']
    output += ' " style = "width: 300px; height: 300px;border-radius: 150px;\
              -webkit-border-radius: 150px;-moz-border-radius: 150px;">1 '
...
def newCatalog():
    """
        Create new catalog, per the users input.

    Result:
        On GET: Page to create new catalog.
        ON post: redirects user to main page after catalog has been created.
        Login page when user not signed in.
    """
...
def editCatalog(catalog_id):
    """
        Edit catalog, per the users input.
    Arg:
        catalog_id: catalog to be edited.
    Result:
        On GET: Page to edit catalog.
        ON post: redirects user to main page after catalog has been edited.
        Login page when user not signed in.
    """
...
def deleteCatalog(catalog_id):
    """
        Delete catalog.
    Arg:
        catalog_id: catalog to be deleted.
    Result:
        On GET: Page to delete catalog.
        ON post: redirects user to main page after catalog has been deleted.
        Login page when user not signed in.
    """
...
def newMenuItem(catalog_id):
    """
        New menu for catalog.
    Arg:
        catalog_id: catalog to add the new menu in.
    Result:
        On GET: Page to create new menu.
        ON post: redirects user to catalog page after menu has been created.
        Login page when user not signed in.
    """
    catalog = session.query(Catalog).filter_by(id=catalog_id).one()
    if login_session['user_id'] != catalog.user_id:
        return "<script>function myFunction() {alert('You are not authorized \
               to add menu items to this catalog. Please create your own \
               catalog in order to add items.');}\
               </script><body onload='myFunction()''>"
    if request.method == 'POST':
...
def editMenuItem(catalog_id, menu_id):
    """
        Edit menu for catalog.
    Arg:
        catalog_id: catalog includes the menu to be edited.
        menu_id: menu to be edited.
    Result:
        On GET: Page to edit menu.
        ON post: redirects user to catalog page after menu has been edited.
        Login page when user not signed in.
    """
    ...
    if login_session['user_id'] != catalog.user_id:
        return "<script>function myFunction()\
                ...
                catalog. Please create your own catalog in order to edit\
...
def deleteMenuItem(catalog_id, menu_id):
    """
        Delete menu for catalog.
    Arg:
        catalog_id: catalog includes the menu to be deleted.
        menu_id: menu to be deleted.
    Result:
        On GET: Page to delete menu.
        ON post: redirects user to catalog page after menu has been deleted.
        Login page when user not signed in.
    """
    catalog = session.query(Catalog).filter_by(id=catalog_id).one()
    ...
    if login_session['user_id'] != catalog.user_id:
        return "<script>function myFunction()\
                ...
                catalog.Please create your own catalog in order to delete \
                items.');}</script><body onload='myFunction()''>"
    if request.method == 'POST':
